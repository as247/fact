 ...
 */
$processHoursFunction = function (\As247\Fact\Carbon\CarbonInterface $date, string $format) {
...
return [
    ...
    'calendar' => [
        'sameDay' => function (\As247\Fact\Carbon\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Сьогодні ');
        },
        'nextDay' => function (\As247\Fact\Carbon\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Завтра ');
        },
        'nextWeek' => function (\As247\Fact\Carbon\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[У] dddd [');
        },
        'lastDay' => function (\As247\Fact\Carbon\CarbonInterface $date) use ($processHoursFunction) {
            return $processHoursFunction($date, '[Вчора ');
        },
        'lastWeek' => function (\As247\Fact\Carbon\CarbonInterface $date) use ($processHoursFunction) {
    ...
    'weekdays' => function (\As247\Fact\Carbon\CarbonInterface $date, $format, $index) {
        static $words = [
